  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Trigger task refresh if action performed
        if (data.action) {
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-80 max-w-[calc(100vw-3rem)] bg-white rounded-xl shadow-2xl overflow-hidden animate-slide-up">
          ...
          <div className="px-3 py-2 bg-gray-50 border-b flex flex-wrap gap-1">
            <button onClick={() => setInput('Add task: ')} className="px-2 py-1 bg-green-100 text-green-700 rounded text-xs hover:bg-green-200 transition-colors">
              ➕ Add
            </button>
            <button onClick={() => setInput('Show my tasks')} className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs hover:bg-blue-200 transition-colors">
              📋 Tasks
            </button>
            <button onClick={() => setInput('Complete task')} className="px-2 py-1 bg-purple-100 text-purple-700 rounded text-xs hover:bg-purple-200 transition-colors">
              ✅ Done
            </button>
          </div>
