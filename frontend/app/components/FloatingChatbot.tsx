...
export default function FloatingChatbot() {
  ...
  const loadChatHistory = async () => {
    try {
      ...
      if (!token) {
        // Show welcome message if no token
        setMessages([{
          id: 'welcome',
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.action) {
          ...
          console.log('✅ Task action completed, refreshing tasks:', data.action);
        }
      } else {
        setMessages(prev => [...prev, {
          id: 'error',
  ...
  const clearChat = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'}/api/chatbot/messages`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-80 max-w-[calc(100vw-3rem)] bg-white rounded-xl shadow-2xl overflow-hidden animate-slide-up">
          ...
          <div className="px-3 py-2 bg-gray-50 border-b flex flex-wrap gap-1">
            <button onClick={() => handleQuickAction('Add task: ')} className="px-2 py-1 bg-green-100 text-green-700 rounded text-xs hover:bg-green-200 transition-colors">
              ➕ Add
            </button>
            <button onClick={() => handleQuickAction('Show my tasks')} className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs hover:bg-blue-200 transition-colors">
              📋 Tasks
            </button>
            <button onClick={() => handleQuickAction('Complete task')} className="px-2 py-1 bg-purple-100 text-purple-700 rounded text-xs hover:bg-purple-200 transition-colors">
              ✅ Done
            </button>
          </div>

          {/* Messages */}
          <div className="h-80 overflow-y-auto p-3 space-y-3 bg-gray-50 scroll-smooth">
            {messages.map((msg) => (
              <div
                key={msg.id}
          ...
          <div className="p-3 border-t bg-white">
            <div className="flex gap-2">
              <input
                ...
                disabled={isLoading}
                className="flex-1 px-3 py-2 border rounded-lg text-sm focus:outline-none focus:border-purple-500 transition-colors"
              />
              <button
                onClick={sendMessage}
                disabled={!input.trim() || isLoading}
                className="px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-lg text-sm hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                Send
              </button>
            </div>
